...
public class Reader {
  ...
  public Reader() {
    try {
      ...
      do {
        ...
        if (!tree.verificaSeEstaNaRaiz()) throw new Exception("Error Running arround the Tree!");
  ...
  public Double media(ArrayList<Integer> m) {
    int i;
    Double sum = 0.0;
    for (i = 0; i < m.size(); i++) sum += m.get(i);
    return sum / m.size();
  }

  public BufferedReader createReaderFromFile(String nomeArquivo) throws FileNotFoundException {
    File arquivo = new File(nomeArquivo);
    if (arquivo.exists()) return new BufferedReader(new FileReader(arquivo));
